...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#050507] text-white">
      <div className="bg-[#0a0a0c] border-b border-white/10 py-2 px-6 text-[11px] flex gap-6 whitespace-nowrap overflow-hidden"><span className="text-gray-500">LIVE:</span><span>BTC ${btc.toFixed(2)}</span><span>ETH $3,421</span><span className="text-gray-600">Education only</span></div>
      ...
      <section className="px-6 pt-16 pb-10 max-w-7xl mx-auto grid lg:grid-cols-2 gap-12">
        <div>
          ...
          <div className="bg-white/[0.04] border border-white/10 rounded-[24px] p-6 mt-8">
        ...
        <div className="grid md:grid-cols-3 gap-4 max-w-6xl mx-auto">
          {reviews.map(r=>(
            <div key={r.name} className="bg-[#0f0f10] border border-white/10 rounded-2xl p-6">
              <div className="flex justify-between mb-3"><span className="text-yellow-400 text-xs">{"★".repeat(r.stars)}</span><span className="text-[10px] text-gray-500 bg-white/10 px-2 py-1 rounded-full">{r.course}</span></div>
              <p className="text-sm leading-6 text-gray-300 mb-4">"{r.text}"</p>
              <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-white text-black flex items-center justify-center font-black text-xs">{r.name[0]}</div><div><p className="text-xs font-bold">{r.name}</p><p className="text-[10px] text-gray-500">{r.loc}</p></div></div>
            </div>
          ))}
        </div>
      </section>
      <footer className="border-t border-white/10 py-8 px-6 text-center text-[10px] text-gray-600">© 2026 Grayscale Research LLC • WhatsApp: +234 911 643 8322</footer>
    </main>
  );
}
